...
export default function CampDashboard() {
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      const r = await api.get("treks/dashboard/");
      setData(r.data);
    } catch (err) {
      console.error(err);
  ...
  const { stats, revenueChart, statusDistribution, topTreks, recentBookings } = data;
  ...
  return (
    <div className="animate-fade">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "24px", marginBottom: "40px" }}>
        {[
          { l: "ACTIVE TREKS", v: stats.total_treks, i: "🏔️", c: "#f97316" },
